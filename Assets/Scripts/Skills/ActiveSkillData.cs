...
public class ActiveSkillData : SkillData
{
    ...
    public class ActiveBranchData
    {
        ...
        [Tooltip("冲刺距离(米;树B冲刺用,直接填距离常量;0 = 未配置回退 PlayerDash 序列化值)")]
        public float dashDistance;           // 树B 冲刺距离(米;0 = 未配置,回退 PlayerDash 序列化)
        [Tooltip("冲刺总持续时间(秒;树B冲刺用,独立设置,随技能等级升级可调;0 = 未配置回退 PlayerDash 序列化值)")]
        public float dashDuration;           // 树B 冲刺总时长(0 = 未配置,回退 PlayerDash 序列化)
