...
/// <summary>
/// 地面移动状态 — grounded 且 |h|&gt;0.1
/// 切换 → Idle(停) / Jump(空格) / Fall(离地)
/// 物理执行(移动)由 PlayerController.OnFixedUpdate 处理,本状态只做调度判断
/// </summary>
public class PlayerMoveState : EntityState
{
    private readonly PlayerJump jump;

    public PlayerMoveState(CharacterBase owner, StateMachine stateMachine, Animator anim, PlayerJump jump)
    ...
    public override void OnUpdate()
    {
        ...
        // 停止 → Idle
        if (Mathf.Abs(h) <= 0.1f)
            stateMachine.ChangeState(pc.IdleState);
    }
}
